...
def _synthetic_envelope() -> np.ndarray:
    ...
    quad = np.array(
        [[180, 140], [1210, 170], [1170, 870], [150, 850]],
        dtype=np.int32,
    )
...
def _synthetic_conveyor_frame() -> np.ndarray:
    ...
    quad = np.array(
        [[100, 230], [1510, 240], [1510, 1199], [95, 1199]],
        dtype=np.int32,
    )
...
def test_detects_external_quad_before_internal_lines():
    ...
    assert detection.raw_confidence >= detection.confidence
    assert detection.frame_status == "full_frame"
    assert detection.frame_contact_sides == ()
    assert detection.area_ratio > 0.40
    assert detection.rectangularity > 0.85
    assert detection.points.shape == (4, 2)


def test_light_envelope_on_dark_conveyor_marks_bottom_frame_contact():
    image = _synthetic_conveyor_frame()
    detection = detect_envelope_quad(image)
    assert detection.method == "foreground_otsu"
    assert detection.raw_confidence > 0.90
    assert detection.confidence < detection.raw_confidence
    assert detection.confidence > 0.75
    assert detection.frame_status == "partial_frame"
    assert detection.frame_contact_sides == ("bottom",)
...
def test_180_degree_input_keeps_same_format_candidates():
    ...
    first_formats = {
        item.format
        for item in candidate_formats_by_aspect_ratio(first.width_px, first.height_px, max_relative_error=0.08)
    }
    second_formats = {
        item.format
        for item in candidate_formats_by_aspect_ratio(second.width_px, second.height_px, max_relative_error=0.08)
    }
