...
`contour_approx` — предпочтительный режим: найден настоящий выпуклый четырёхугольник внешнего контура.

`min_area_rect_fallback` — запасной режим для слабой границы. Его confidence специально ограничен; такие случаи желательно отдельно проверить визуально.
...
Для production это правильнее, чем продолжать OCR по случайному crop.
